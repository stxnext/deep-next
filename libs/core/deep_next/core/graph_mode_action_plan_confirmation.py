...
class DeepNextActionPlanGraph(BaseGraph):
    ...
    def __call__(
        ...
        return _StateActionPlan.model_validate(final_state).action_plan
...
class DeepNextImplementGraph(BaseGraph):
    ...
    def _build(self):
        self.add_node(_StateImplement.implement)

        self.add_quick_edge(START, _StateImplement.implement)
        self.add_quick_edge(_StateImplement.implement, END)
