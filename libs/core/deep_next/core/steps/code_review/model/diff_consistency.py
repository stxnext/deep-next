...
_issues_desc = (
    ...
    "consistency, that is:"
    "\n- Situations, when a newly created or already existing utility, function, "
    "class or other piece of code could be reused, but is not."
    "\n- Situations, when a newly created or already existing utility, function, "
    "class or other piece of code is used with a mistake, e.g.: when a method is "
    "misspelled, or it seems like a developer was trying to GUESS the correct name of "
    "a function that in reality is named differently."
    "\n- Situations, when a newly created or already existing utility, function, "
    "class or other piece of code is used with incorrect arguments, a wrong number of "
    "arguments, or in a wrong context."
    "\n- Situations, when a newly created or already existing utility, function, "
    "class or other piece of code is used to obtain a certain return value which in "
    "reality is not at all returned by the used entity, e.g.: when a function returns "
    "a list, but the developer is trying to use it as a dictionary, or when two values "
    "are returned, but the developer is trying to unpack it as three values."
    "\n"
